...
export class TrajectoryService {
    ...
    private readonly ZUPT_MIN_CONSECUTIVE = 100; // Require 100ms of stable rest before triggering
    ...
    private readonly REST_THRESHOLD = 0.06; // rad/s threshold for gyroscope to detect rest (~3.4 deg/s)
    private readonly ACCEL_REST_WINDOW = 0.03; // g deviation from 1g allowed for rest
     ...
     */
    reset() {
        console.log('[RESET] Trajectory state reset (orientation, velocity, position).');
    ...
    processSample(sample: IMUSample): TrajectoryPoint {
        ...
        // 3. Compute corrected orientation using initial quaternion
        // qCorr = q * conj(initialQ) aligns sensor frame with world "up"
        // 3. Compute corrected orientation using initial quaternion
        // qCorr = q * conj(initialQ) aligns sensor frame with world "up"
        const q0_corr = this.initialQ;
        const q0Conj_corr = { w: q0_corr.w, x: -q0_corr.x, y: -q0_corr.y, z: -q0_corr.z };
        const qCorr = {
            w: this.q.w * q0Conj_corr.w - this.q.x * q0Conj_corr.x - this.q.y * q0Conj_corr.y - this.q.z * q0Conj_corr.z,
            x: this.q.w * q0Conj_corr.x + this.q.x * q0Conj_corr.w + this.q.y * q0Conj_corr.z - this.q.z * q0Conj_corr.y,
            y: this.q.w * q0Conj_corr.y - this.q.x * q0Conj_corr.z + this.q.y * q0Conj_corr.w + this.q.z * q0Conj_corr.x,
            z: this.q.w * q0Conj_corr.z + this.q.x * q0Conj_corr.y - this.q.y * q0Conj_corr.x + this.q.z * q0Conj_corr.w
        };
        // 4. Rotate Accel to World Frame using corrected orientation
        const x = qCorr.x, y = qCorr.y, z = qCorr.z, w = qCorr.w;
        ...
        // 5. Remove Gravity using corrected orientation
        ...
        // Determine stationary state using averaged values
        const isStationary = avgGyroMag < this.REST_THRESHOLD && Math.abs(avgAccelMag - 1.0) < this.ACCEL_REST_WINDOW;

        // Increment or reset consecutive counter based on stationary detection
        if (isStationary) {
        ...
        // ZUPT triggers only after required consecutive stationary samples
        const stationaryNow = this.zuptConsecutiveCount >= this.ZUPT_MIN_CONSECUTIVE;
        ...
        if (isStationary) {
            if (this.isMoving) {
                ...
                console.log('[ZUPT] STOP detected. Computing drift correction.', { vDriftX, vDriftY, vDriftZ });

                // Retroactive Quadratic Correction before zeroing velocity
            ...
            if (!this.isMoving) {
                this.isMoving = true;
                this.segmentStartIndex = this.path.length;
        ...
        */
        ...
        // q_rel = q * q0_conj (for UI visualization)
        // q_rel = q * q0_conj (for UI visualization)
        const q0_rel = this.initialQ;
        const q0Conj_rel = { w: q0_rel.w, x: -q0_rel.x, y: -q0_rel.y, z: -q0_rel.z };
        const qRel = {
            w: this.q.w * q0Conj_rel.w - this.q.x * q0Conj_rel.x - this.q.y * q0Conj_rel.y - this.q.z * q0Conj_rel.z,
            x: this.q.w * q0Conj_rel.x + this.q.x * q0Conj_rel.w + this.q.y * q0Conj_rel.z - this.q.z * q0Conj_rel.y,
            y: this.q.w * q0Conj_rel.y - this.q.x * q0Conj_rel.z + this.q.y * q0Conj_rel.w + this.q.z * q0Conj_rel.x,
            z: this.q.w * q0Conj_rel.z + this.q.x * q0Conj_rel.y - this.q.y * q0Conj_rel.x + this.q.z * q0Conj_rel.w
        };
        console.log('[ORIENTATION] Relative quaternion (qRel) computed.', qRel);
        console.log('[ORIENTATION] Relative quaternion (qRel) computed.', qRel);
    ...
    async calibrateAsync(durationMs: number = 2000): Promise<void> {
        ...
        if (this.calibrationBuffer.length > 0) {
            ...
            console.log('[CALIBRATION] Captured initial orientation quaternion (q0).', this.initialQ);
